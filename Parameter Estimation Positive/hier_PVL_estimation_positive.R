...
setwd('/work/AlinaMariaNechita#3266/PE2')
...
#load data
positive_data <- read.csv("parameter_estimation.csv", sep=";")

# filter participants who had a negative Net_Outcome and 0 Outcome in the first trial
first_trial_positive <- positive_data[positive_data$Trial == 1, ]
positive_participants <- first_trial_positive[first_trial_positive$Net_Outcome > 0, "Subj_ID"]


positive_data <- positive_data[positive_data$Subj_ID %in% positive_participants, ]
cat("Total participants included in the analysis:", length(unique(positive_data$Subj_ID)), "\n")
#----------prepare data for jags models - want trial x subject arrays for choice and gain & loss ----
# identify and count unique subject IDs
subIDs <- unique(positive_data$Subj_ID)
nsubs <- length(subIDs)
ntrials_max <- 100

# all choices (x) and outcomes (X)
x_raw <- positive_data$Choice
X_raw <- positive_data$Wins + positive_data$Losses #note the sign!
...
for (s in 1:nsubs) {
  ...
  ntrials_all[s] <- length(x_raw[positive_data$Subj_ID==subIDs[s]])
  
  #pad trials with NA if n trials < maximum (i.e. 100)
  x_sub <- x_raw[positive_data$Subj_ID==subIDs[s]] 
...
save(samples,
     file = "output_data/hier_est_positive_good.RData")
...
ggsave('pl1_mu_w_good.png', pl1)
ggsave('pl2_mu_A_good.png', pl2)
ggsave('pl3_mu_theta_good.png', pl3)
ggsave('pl4_mu_a_good.png', pl4)
...
ggsave('pl1_lambda_w_good.png', pl1_1)
ggsave('pl2_lambda_A_good.png', pl2_2)
ggsave('pl3_lambda_theta_good.png', pl3_3)
ggsave('pl4_lambda_a_good.png', pl4_4)
