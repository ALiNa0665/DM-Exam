...
nsubs <- 30 # mimicking the data structure from the paper
ntrials_all <- rep(100, 30) # all 30 subs have 100 trials each
...
for (i in 1:niterations) {
  ...
  save(samples,
       file = "output_data/hier_recovery_30_new.RData")
   # mu
  true_mu_w[i] <- mu_w
  true_mu_A[i] <- mu_A
...
ggsave('pl1_mu_new.png', pl1)
ggsave('pl2_mu_new.png', pl2)
ggsave('pl3_mu_new.png', pl3)
ggsave('pl4_mu_new.png', pl4)

# sigma (aka. true_lambda) re-coded as precision
pl1_1 <- recov_plot(infer_lambda_w, 1/(true_lambda_w^2), c("infer lambda_w","true lambda_w"), 'smoothed linear fit')
pl2_2 <- recov_plot(infer_lambda_A, 1/(true_lambda_A^2), c("infer lambda_A","true lambda_A"), 'smoothed linear fit')
pl3_3 <- recov_plot(infer_lambda_theta, 1/(true_lambda_theta^2), c("infer lambda_theta", "true lambda_theta"), 'smoothed linear fit')
pl4_4 <- recov_plot(infer_lambda_a, 1/(true_lambda_a^2), c("infer lambda_a", "true lambda_a"), 'smoothed linear fit')
ggarrange(pl1, pl2, pl3, pl4)
ggsave('pl1_lambda_new.png', pl1_1)
ggsave('pl2_lambda_new.png', pl2_2)
ggsave('pl3_lambda_new.png', pl3_3)
ggsave('pl4_lambda_new.png', pl4_4)


# lambda (aka. infer_lambda) re-coded as SD
pl1_1_SD <- recov_plot(1/sqrt(infer_lambda_w), true_lambda_w, c("infer lambda_w","true lambda_w"), 'smoothed linear fit')
pl2_2_SD <- recov_plot(1/sqrt(infer_lambda_A), true_lambda_A, c("infer lambda_A","true lambda_A"), 'smoothed linear fit')
pl3_3_SD <- recov_plot(1/sqrt(infer_lambda_theta), true_lambda_theta, c("infer lambda_theta", "true lambda_theta"), 'smoothed linear fit')
pl4_4_SD <- recov_plot(1/sqrt(infer_lambda_a), true_lambda_a, c("infer lambda_a", "true lambda_a"), 'smoothed linear fit')
ggarrange(pl1, pl2, pl3, pl4)
ggsave('pl1_lambdaSD_new.png', pl1_1_SD)
ggsave('pl2_lambdaSD_new.png', pl2_2_SD)
ggsave('pl3_lambdaSD_new.png', pl3_3_SD)
ggsave('pl4_lambdaSD_new.png', pl4_4_SD)
...
save(ntrials_all, 
     ...
     infer_lambda_a,
     true_lambda_a,
     file="JAGS_hier_PVL_recovery_new.RData")
print('saving recovery data new')
