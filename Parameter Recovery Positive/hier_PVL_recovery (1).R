...
setwd('/work/AlinaMariaNechita#3266/PVL2')
...
nsubs <- 110 # mimicking the data structure from the paper
ntrials_all <- rep(100, 110) # all 110 subs have 100 trials each
...
for (i in 1:niterations) {
  ...
  save(samples,
       file = "output_data/hier_recovery_110_new.RData")
  # mu
  true_mu_w[i] <- mu_w
  true_mu_A[i] <- mu_A
...
ggsave('pl11_mu_new.png', pl1)
ggsave('pl22_mu_new.png', pl2)
ggsave('pl33_mu_new.png', pl3)
ggsave('pl44_mu_new.png', pl4)

# sigma (aka. true_lambda) re-coded as precision
pl11_1 <- recov_plot(infer_lambda_w, 1/(true_lambda_w^2), c("infer lambda_w","true lambda_w"), 'smoothed linear fit')
pl22_2 <- recov_plot(infer_lambda_A, 1/(true_lambda_A^2), c("infer lambda_A","true lambda_A"), 'smoothed linear fit')
pl33_3 <- recov_plot(infer_lambda_theta, 1/(true_lambda_theta^2), c("infer lambda_theta", "true lambda_theta"), 'smoothed linear fit')
pl44_4 <- recov_plot(infer_lambda_a, 1/(true_lambda_a^2), c("infer lambda_a", "true lambda_a"), 'smoothed linear fit')
ggarrange(pl1, pl2, pl3, pl4)
ggsave('pl11_lambda_new.png', pl11_1)
ggsave('pl22_lambda_new.png', pl22_2)
ggsave('pl33_lambda_new.png', pl33_3)
ggsave('pl44_lambda_new.png', pl44_4)


# lambda (aka. infer_lambda) re-coded as SD
pl11_1_SD <- recov_plot(1/sqrt(infer_lambda_w), true_lambda_w, c("infer lambda_w","true lambda_w"), 'smoothed linear fit')
pl22_2_SD <- recov_plot(1/sqrt(infer_lambda_A), true_lambda_A, c("infer lambda_A","true lambda_A"), 'smoothed linear fit')
pl33_3_SD <- recov_plot(1/sqrt(infer_lambda_theta), true_lambda_theta, c("infer lambda_theta", "true lambda_theta"), 'smoothed linear fit')
pl44_4_SD <- recov_plot(1/sqrt(infer_lambda_a), true_lambda_a, c("infer lambda_a", "true lambda_a"), 'smoothed linear fit')
ggarrange(pl1, pl2, pl3, pl4)
ggsave('pl11_lambdaSD_new.png', pl11_1_SD)
ggsave('pl22_lambdaSD_new.png', pl22_2_SD)
ggsave('pl33_lambdaSD_new.png', pl33_3_SD)
ggsave('pl44_lambdaSD_new.png', pl44_4_SD)
...
save(ntrials_all, 
     ...
     infer_lambda_a,
     true_lambda_a,
     file="JAGS_hier_PVL_recovery_two_new.RData")
print('saving recovery data second new')